...
The plot highlights the variables that contribute to the positive end of cline like adverbs, *to be* as a main verb, and first-person pronouns. At the other end, are nounts, longer words, more atttributive adjectives, and prepositions.
...
```{r}
f1_aov <- aov(Factor1 ~ group, data = m)
f2_aov <- aov(Factor2 ~ group, data = m)
```

```{r}
data.frame(Factor1 = t(broom::glance(f1_lm)), Factor2 = t(broom::glance(f2_lm))) %>% knitr::kable()
```
